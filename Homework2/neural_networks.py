...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        W = self.params['W']
        b = self.params['b']
        backward_output = np.matmul(grad,np.transpose(W))
        self.gradient['W'] = np.matmul(np.transpose(X),grad)
        self.gradient['b'] = np.matmul(np.ones((1,X.shape[0])),grad)
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        backward_output = grad * np.where(X < 0,0,1)
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    momentum[module_name + '_' + key] = momentum[module_name + '_' + key] * _alpha - _learning_rate * g
                    ...
                    #################################################################################
                    # model[module_name].params[key] = module.params[key] - _learning_rate * g
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    _step = 10
    _alpha = float(main_params['alpha']) # discount factor for velocity in momentum
