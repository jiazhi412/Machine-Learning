...
class KNN:
    ...
    def train(self, features, labels):
        ...
        """
        # self.features = np.delete(features,0,axis=1)
    ...
    def predict(self, features):
        ...
        for i in range(len(features)):
            ones = 0
            zeros = 0
            k_neighbor = self.get_k_neighbors(features[i])
    ...
    def get_k_neighbors(self, point):
        ...
        :return:  List[int]
        """
        # calculte all distance for this point
        dists = []
        features = self.features
        distance_function = self.distance_function
        for i in range(len(features)):
            dist = distance_function(point,features[i])
            dists.append(dist)

        # select first k features for this point
        dists = np.array(dists)
        sortedIndices = dists.argsort()
        indices = sortedIndices[:self.k]
        res = []
        for i in indices:
